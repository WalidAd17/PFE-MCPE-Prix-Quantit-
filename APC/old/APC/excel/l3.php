...
$c1=$_GET['s1'];
$c3=$_GET['s2'];
  $p= "select * from structure where cod='".$c3."'";
   include "../connection.php";
 //echo $p;
 $rg=$mysqli->query($p);
if($rr= mysqli_fetch_array($rg) ){
  echo'<div align="center"><h3>  &nbsp;&nbsp;Prix journalier des Produits de large consommation du: '.$c1.' au niveau de dcw '.$rr['desfr'].'</h3></div>';
...
$req= $mysqli->query( 'select detail,grox,mind,maxd,date,(produit.nomfr)as nom ,produit.unite  from prixjour,produit where date='."'".$c1."'".' and produit.codpro=codp  and strect='."'".$c3."' order by produit.type asc,produit.codpro asc");
//$req= $mysqli->query( 'select cast(avg(detail)AS DECIMAL(10,2)) as detail1,cast(avg(grox)AS DECIMAL(10,2))as grox1,cast(avg(mind)AS DECIMAL(10,2))as min, cast(avg(maxd)AS DECIMAL(10,2)) as max ,cast(avg(grox)AS DECIMAL(10,2))as grox1 ,date,(produit.nomfr)as nom ,produit.unite , structure.reg from prixjour,produit, structure where structure.cod=strect and  structure.reg =  '."'".$c3."' ".' and date='."'".$c1."'".' and produit.codpro=codp and detail>0 and grox>0  group by codp order by produit.type asc,produit.codpro asc');
...
//$ma= $mysqli->query( 'select cast(avg(maxd)AS DECIMAL(10,2)) as max,date,(produit.nomfr)as nom ,produit.unite  from prixjour,produit where date='."'".$c1."'".$c3.' and produit.codpro=codp and maxd>0 and grox>0 and codp='.$r1['codp'].' group by codp');
							...
							<td ><?php  echo $r1['nom'];  ?></td>
								...
								<td ><?php echo $r1['mind']; ?></td>
								<td ><?php echo $r1['maxd']; ?></td>
								</tr>
							<?php } 
    print '</TABLE>';
