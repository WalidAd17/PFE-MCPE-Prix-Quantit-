...
header("Content-Disposition: attachment; filename=prix".$c1.".xls");
    // � elle seule, la ligne suivante suffit � envoyer le r�sultat du script dans une feuille Excel
 $p= "select * from structure where cod='".$t."'";
   include "../connection.php";
 //echo $p;
 $rg=$mysqli->query($p);
if($rr= mysqli_fetch_array($rg) ){
  echo'<div align="center"><h3>  &nbsp;&nbsp;Etat du suivi des Prix de reference des produits pour la journée du '.$c1.' au niveau de  '.$rr['desfr'].'</h3></div>';
...
//$req= $mysqli->query( 'select cast(avg(detail)AS DECIMAL(10,2)) as detail1  ,cast(avg(grox)AS DECIMAL(10,2))as grox1 ,date,(produit.nomfr)as nom ,produit.unite , structure.reg , prref.maxg,prref.ming,prref.min,prref.max  from prixjour,produit,prref, structure where structure.cod=strect and  structure.reg =  '."'".$t."' ".' and date='."'".$c1."'".' and produit.codpro=codp and detail>0 and grox>0 and codp=prref.produit  group by codp order by produit.type asc,produit.codpro asc');
 $req= $mysqli->query( 'select detail,grox,mind,maxd,date,(produit.nomfr)as nom ,produit.unite,prref.min,prref.max,prref.maxg,prref.ming  from prixjour,produit,prref where date='."'".$c1."'".' and produit.codpro=codp  and strect='."'".$t."' and codp=prref.produit order by produit.type asc,produit.codpro asc");  
 ...
 <TR><TD>Produits</TD><TD>Prix de ref gros</TD><TD>	Prix de gros</TD><TD>Ecart</TD><TD>Prix de ref d�tail</TD><TD>Prix de detail</TD><TD>Ecart</TD></TR>';    // lecture du contenu de la requ�te avec 2 boucles imbriqu�es; par ligne et par colonne
   while ($r1=mysqli_fetch_array($req)) {
							...
							<td ><?php  echo $r1['nom'];  ?></td>
									...
									<td ><?php  echo $r1['grox']; ?></td>
									<td ><?php echo $r1['grox']- $r1['maxg']?> </td>
								<td ><?php if ($r1['min']== $r1['max']){echo $r1['min'];} else{echo $r1['min']." - ".$r1['max'];}  ?></td>
								<td ><?php  echo $r1['detail'];  ?></td>
								<td ><?php echo $r1['detail']- $r1['max']?></td>
							</tr>
							<?php } 
    print '</TABLE>';
